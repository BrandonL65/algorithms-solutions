...
const minimumWaitingTime = (queries: number[]) => {
  queries.sort((a, b) => a - b);

  let totalWaitingTime = 0;

  let runningTotalOfWaitingTime = 0;

  for (let taskTime of queries) {
    totalWaitingTime += runningTotalOfWaitingTime;
    runningTotalOfWaitingTime += taskTime;
  }
